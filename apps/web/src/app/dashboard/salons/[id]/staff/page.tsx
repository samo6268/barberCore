    ...
    <DashboardLayout salonId={id} activeTab="staff">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold" style={{ color: 'var(--color-text)' }}>تیم متخصصین</h2>
        ...
        {showForm && (
          <div className="rounded-2xl p-5 border space-y-3" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            ...
            <div className="flex gap-2">
              <button onClick={handleCreate} disabled={createStaff.isPending}
                className="px-6 py-2 rounded-xl text-white text-sm font-medium disabled:opacity-50" style={{ background: 'var(--color-primary)' }}>
        ...
        {isLoading ? (
          <div className="grid grid-cols-2 gap-4">{Array(4).fill(0).map((_, i) => <div key={i} className="h-32 rounded-2xl animate-pulse" style={{ background: 'var(--color-surface)' }} />)}</div>
        ) : !staff?.length ? (
          <div className="text-center py-16 rounded-2xl border" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            <div className="text-5xl mb-3">👤</div>
            <p style={{ color: 'var(--color-muted)' }}>هنوز متخصصی اضافه نکرده‌اید</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {staff.map((s: any) => (
              <div key={s.id} className="flex items-center gap-3 p-4 rounded-2xl border" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
                <div className="w-14 h-14 rounded-xl overflow-hidden shrink-0" style={{ background: 'var(--color-background)' }}>
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-sm truncate" style={{ color: 'var(--color-text)' }}>{s.displayName}</p>
                  {s.specialties?.length > 0 && <p className="text-xs mt-0.5 truncate" style={{ color: 'var(--color-muted)' }}>{s.specialties.join('، ')}</p>}
                  <p className="text-xs mt-1" style={{ color: 'var(--color-muted)' }}>{s.services?.length || 0} خدمت</p>
                </div>
              </div>
            ))}
          </div>
