  ...
  return (
    <DashboardLayout salonId={id} activeTab="settings">
      <div className="max-w-xl space-y-6">
        <h2 className="text-xl font-bold" style={{ color: 'var(--color-text)' }}>تنظیمات سالن</h2>

        <div className="rounded-2xl p-6 border space-y-4" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
          <h3 className="font-semibold" style={{ color: 'var(--color-text)' }}>اطلاعات اصلی</h3>
          {[
            ...
            <div key={f.key}>
              <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-text)' }}>{f.label}</label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-text)' }}>توضیحات</label>
            <textarea value={form.description} onChange={e => setForm(p => ({...p, description: e.target.value}))} rows={3}
              className="w-full px-4 py-2 rounded-xl border text-sm outline-none resize-none"
              style={{ borderColor: 'var(--color-border)', background: 'var(--color-background)', color: 'var(--color-text)' }} />
          </div>
          <button onClick={handleSave} className="w-full py-3 rounded-xl text-white font-medium" style={{ background: 'var(--color-primary)' }}>
            ذخیره تغییرات
          </button>
        </div>
        ...
        <div className="rounded-2xl p-6 border" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
          <h3 className="font-semibold mb-4" style={{ color: 'var(--color-text)' }}>ساعات کاری</h3>
          <div className="space-y-2">
            {salon?.workingHours?.map((wh: any) => (
              <div key={wh.id} className="flex items-center justify-between py-2 border-b last:border-0" style={{ borderColor: 'var(--color-border)' }}>
                <span className="text-sm font-medium" style={{ color: 'var(--color-text)' }}>{DAYS.find(d => d.key === wh.dayOfWeek)?.label}</span>
                {wh.isOpen ? (
                  <span className="text-sm" dir="ltr" style={{ color: 'var(--color-muted)' }}>{wh.openTime} – {wh.closeTime}</span>
                ) : (
