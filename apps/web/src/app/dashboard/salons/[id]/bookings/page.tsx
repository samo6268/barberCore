    ...
    <DashboardLayout salonId={id} activeTab="bookings">
      <div className="space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <h2 className="text-xl font-bold" style={{ color: 'var(--color-text)' }}>رزروها</h2>
        ...
        {isLoading ? (
          <div className="space-y-3">{Array(5).fill(0).map((_, i) => <div key={i} className="h-20 rounded-xl animate-pulse" style={{ background: 'var(--color-surface)' }} />)}</div>
        ) : !bookings?.length ? (
          <div className="text-center py-16 rounded-2xl border" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            <div className="text-5xl mb-3">📭</div>
            <p style={{ color: 'var(--color-muted)' }}>در این روز رزروی وجود ندارد</p>
          </div>
        ) : (
          <div className="space-y-3">
            {bookings.map((b: any) => {
              ...
              return (
                <div key={b.id} className="flex items-center gap-4 p-4 rounded-2xl border" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
                  <div className="w-14 text-center">
                    <p className="font-bold text-sm" dir="ltr" style={{ color: 'var(--color-primary)' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {b.staff && <p className="text-xs" style={{ color: 'var(--color-muted)' }}>👤 {b.staff.displayName}</p>}
                  </div>
                  <div className="text-left shrink-0">
                    <span className="text-xs font-medium px-2 py-1 rounded-full" style={{ color, background: `${color}18` }}>
                      {STATUS_LABELS[b.status] || b.status}
                    </span>
                    <p className="text-sm font-semibold mt-1" style={{ color: 'var(--color-primary)' }}>{formatPrice(b.totalPrice)}</p>
                  </div>
                </div>
              );
            })}
