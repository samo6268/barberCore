...
export default function CustomerLoginPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      toast.success('کد تأیید ارسال شد — کد دمو: 123456');
      setStep('code');
    } catch { toast.error('خطا در ارسال کد'); }
  };
  ...
  const demoLogin = async () => {
    try {
      await sendOtp.mutateAsync('09100000001');
      await verifyOtp.mutateAsync({ phone: '09100000001', code: '123456' });
...
شایسته‌اش هستید"
    ...
    >
      ...
      {step === 'phone' ? (
        <div className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-3 border-2 rounded-xl px-4 py-3 transition-colors focus-within:border-[var(--brand-plum-600)]"
          ...
          <button onClick={demoLogin} disabled={verifyOtp.isPending}
        ...
        <div className="space-y-4">
          <p className="text-sm text-center py-3 rounded-xl" style={{ color: 'var(--ui-gray-500)', background: 'var(--bg-ivory)' }}>
            کد ۶ رقمی برای{' '}
            <span className="font-medium" dir="ltr" style={{ color: 'var(--brand-navy-600)' }}>{phone}</span>{' '}
            ارسال شد
          </p>
          ...
          <div>
            ...
            <p className="text-xs text-center mt-2" style={{ color: 'var(--ui-gray-400)' }}>
              کد دمو همیشه <strong>123456</strong> است
            </p>
          </div>
      ...
      <p className="text-center text-xs mt-8" style={{ color: 'var(--ui-gray-400)' }}>
        ورود نشانه‌ی قبول{' '}
        <Link href="#" style={{ color: 'var(--brand-plum-600)' }}>قوانین پرنگارین</Link>
        {' '}است
      </p>
    </AuthLayout>
  );
