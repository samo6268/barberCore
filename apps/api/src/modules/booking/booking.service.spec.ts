...
describe('BookingService', () => {
  ...
  const dto: CreateBookingDto = { salonId: 'salon-1', serviceIds: ['service-1', 'service-2'], date: '2030-01-10', time: '10:00' };
  ...
  beforeEach(() => {
    ...
    prisma.booking.create.mockImplementation(({ data }) => Promise.resolve({ id: 'booking-1', ...data }));
  });
  ...
  it('creates a booking with server-calculated duration and price', async () => {
    ...
    expect(prisma.booking.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({
      customerId: 'customer-1', totalPrice: 1_200_000,
      endsAt: new Date('2030-01-10T11:15:00.000Z'), status: BookingStatus.CONFIRMED,
    }) }));
    expect(booking.id).toBe('booking-1');
  });

  it('rejects duplicate or unknown services', async () => {
    await expect(service.create('customer-1', { ...dto, serviceIds: ['service-1', 'service-1'] })).rejects.toBeInstanceOf(BadRequestException);
    prisma.service.findMany.mockResolvedValueOnce([]);
    await expect(service.create('customer-1', { ...dto, serviceIds: ['unknown'] })).rejects.toBeInstanceOf(BadRequestException);
  });

  it('rejects malformed or past booking times', async () => {
    await expect(service.create('customer-1', { ...dto, date: '2030-02-31' })).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.create('customer-1', { ...dto, date: '2020-01-01' })).rejects.toBeInstanceOf(BadRequestException);
  });
